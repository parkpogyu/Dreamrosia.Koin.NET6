...
namespace Dreamrosia.Koin.Infrastructure.Services.Identity
{
    public class IdentityService : ITokenService
    {
        ...
        public IdentityService(UserManager<BlazorHeroUser> userManager,
        ...
        private async Task<Result<TokenResponse>> GenerateToken(BlazorHeroUser user, DateTime? expires_at = null)
        {
            try
            {
                var token = await GenerateJwtAsync(user, expires_at);
        ...
        public async Task<Result<TokenResponse>> KakaoLoginAsync()
        {
            try
            {
                ...
                var email = external.Principal.Claims.SingleOrDefault(f => f.Type == ClaimTypes.Email)?.Value;

                var user = await _userManager.FindByEmailAsync(email);

                var signed = await _signInManager.ExternalLoginSignInAsync(external.LoginProvider,
                                                                           external.ProviderKey,
                                                                           isPersistent: false,
                                                                           bypassTwoFactor: true);

                // 추후 IsLockedOut, IsNotAllowed 처리
                if (signed.Succeeded)
                {
                    // 카카오 로그인 유효기간 사용
                    string expires_at = external.AuthenticationTokens.SingleOrDefault(f => f.Name.Equals("expires_at"))?.Value;

                    return await GenerateToken(user, string.IsNullOrEmpty(expires_at) ? null : Convert.ToDateTime(expires_at).ToUniversalTime());
                }
                else
                {
                    return await Result<TokenResponse>.FailAsync(_localizer["External Login Failed"]);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task<string> GenerateJwtAsync(BlazorHeroUser user, DateTime? expires_at = null)
        {
            var token = GenerateEncryptedToken(GetSigningCredentials(), await GetClaimsAsync(user), expires_at);
        ...
        private async Task<IEnumerable<Claim>> GetClaimsAsync(BlazorHeroUser user)
        {
            ...
            var claims = new List<Claim>
            {
                ...
                new(ClaimTypes.Name, user.NickName?? string.Empty),
                new(ClaimTypes.Email, user.Email),
                new(ClaimTypes.MobilePhone, user.PhoneNumber ?? string.Empty),
                new(ApplicationClaimTypes.KoreanName, user.KoreanName ?? string.Empty),
                new(ApplicationClaimTypes.ProfielImange, user.ProfileImage ?? string.Empty),
            }
            .Union(userClaims)
            .Union(roleClaims)
        ...
        private string GenerateEncryptedToken(SigningCredentials signingCredentials, IEnumerable<Claim> claims, DateTime? expires_at = null)
        {
            DateTime expires = expires_at ?? DateTime.UtcNow.AddDays(2);
        ...
        private ClaimsPrincipal GetPrincipalFromExpiredToken(string token)
        {
            var tokenValidationParameters = new TokenValidationParameters
            {
                ...
                IssuerSigningKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_serverConfig.Secret)),
        ...
        {
            var secret = Encoding.UTF8.GetBytes(_serverConfig.Secret);
